(() => {
    ...
    function getApplicantsSnapshot() {
        return localStorage.getItem(storageKeys.applicants) || '[]';
    }
    ...
    function viewRequirementFile(fileKey) {
        ...
        const popup = window.open(fileItem.dataUrl, '_blank', 'noopener,noreferrer');
        if (!popup) {
            alert('Pop-up diblokir browser. Izinkan pop-up untuk melihat file.');
        }
    }
    ...
    function ensureAdminAuthenticated() {
        if (!localStorage.getItem(storageKeys.adminLoggedIn)) {
            window.location.href = '../index.html';
        }
    }
