(() => {
    ...
    function applyRemotePayloadToLocal(payload) {
        ...
        CLOUD_SYNC_KEYS.forEach((key) => {
            ...
            const previousRaw = window.ImtekkuStore.getItem(key);
            hasRemoteData = true;
            ...
            window.ImtekkuStore.setItem(key, nextRaw);
            dirtyCloudKeys.delete(key);
    ...
    function setLocalOnlyJson(key, value) {
        window.ImtekkuStore.setItem(key, JSON.stringify(value));
    }
    ...
    function collectLocalCloudPayload() {
        ...
        CLOUD_SYNC_KEYS.forEach((key) => {
            const raw = window.ImtekkuStore.getItem(key);
            if (raw === null) {
                return;
            }
    ...
    async function flushDirtyCloudKeysAsync() {
        ...
        for (const key of keysToFlush) {
            const rawValue = window.ImtekkuStore.getItem(key);
    ...
    function setDefaultDataIfMissing() {
        if (!window.ImtekkuStore.getItem(DATA_KEYS.team)) {
            setLocalOnlyJson(DATA_KEYS.team, INITIAL_TEAM_MEMBERS);
        }
        
        if (!window.ImtekkuStore.getItem(DATA_KEYS.divisions)) {
            const divisions = {};
        ...
        if (!window.ImtekkuStore.getItem(DATA_KEYS.galleryCategories)) {
        ...
        if (!window.ImtekkuStore.getItem(DATA_KEYS.archivedCollections)) {
            setLocalOnlyJson(DATA_KEYS.archivedCollections, INITIAL_ARCHIVED_COLLECTIONS);
        }

        if (!window.ImtekkuStore.getItem(DATA_KEYS.homeSettings)) {
        ...
        if (!window.ImtekkuStore.getItem(DATA_KEYS.recruitmentSettings)) {
    ...
    function shouldRunStorageOptimization() {
        try {
            return window.ImtekkuStore.getItem(STORAGE_OPTIMIZATION_VERSION_KEY) !== STORAGE_OPTIMIZATION_VERSION;
        } catch (error) {
            return false;
        }
    }
    ...
    async function optimizeStorageUsageOnce() {
        ...
        try {
            window.ImtekkuStore.setItem(STORAGE_OPTIMIZATION_VERSION_KEY, STORAGE_OPTIMIZATION_VERSION);
        } catch (error) {
    ...
    function setJson(key, value) {
        try {
            ...
            window.ImtekkuStore.setItem(key, jsonString);
            if (CLOUD_SYNC_KEYS.includes(key)) {
                dirtyCloudKeys.add(key);
            }
